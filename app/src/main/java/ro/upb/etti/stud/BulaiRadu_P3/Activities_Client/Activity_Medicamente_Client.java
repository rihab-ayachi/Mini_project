...
public class Activity_Medicamente_Client extends AppCompatActivity {
    ...
    private void createMedicament(String nom) {
        long id = db.insertMedicament(nom);
    ...
    public void showMedicamenteEditDialog(final boolean shouldUpdate, final Medicamente medicament, final int position) {
        ...
        final EditText inputNom = view.findViewById(R.id.iddialog_medicament);

        TextView dialogTitle = view.findViewById(R.id.iddialog_medicamente_title);
        dialogTitle.setText(!shouldUpdate ? "Ajouter un nouveau médicament" : "Edit medicament");

        if (shouldUpdate && medicament != null) {
            inputNom.setText(medicament.getDenumire());
        }
            ...
            public void onClick(View view) {
                ...
                if (TextUtils.isEmpty(inputNom.getText().toString())) {
                    ...
                    Toast.makeText(Activity_Medicamente_Client.this,
                            "Remplissez le nom du médicament!", Toast.LENGTH_SHORT).show();
                    return;
                } else {
                    alertDialog.dismiss();
                }
                ...
                if (shouldUpdate && medicament != null) {
                    ...
                    updateMedicament(inputNom.getText().toString(),
                            position);
                } else {
                    // create new medicament
                    createMedicament(inputNom.getText().toString());
                }
            }
        });
    }
